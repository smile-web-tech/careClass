...
const PHASE_BADGE = {
  done: { label: 'Done', bg: '#EAF6F1', fg: '#0B7C55' },
  live: { label: 'Now', bg: '#EAF6F1', fg: '#0B7C55' },
  next: { label: 'Next', bg: color.primaryTint, fg: color.primaryInk },
  later: { label: 'Later', bg: color.bg, fg: color.muted },
} as const;
    ...
    <Screen>
      <View style={[styles.header, { paddingTop: insets.top + 6 }]}>
        <View style={styles.headerRow}>
          <View style={{ flex: 1 }}>
            <Text style={text.pageTitle}>{heading}</Text>
            ...
            return (
              <Press
                ...
                <Text
                  style={[
                    styles.dayName,
                    { color: on ? '#fff' : color.inkSoft, opacity: on ? 0.75 : 0.7 },
                  ]}>
                  {dowShort(d)}
                </Text>
          ...
          daySessions.map((s, i) => {
            ...
            return (
              <TimelineRow
                ...
                onNotify={() =>
                  router.push({ pathname: '/compose', params: { group: g.id } })
                }
...
function TimelineRow({
  ...
  const badge = PHASE_BADGE[sessionPhase(session)];
...
const styles = StyleSheet.create({
  ...
  weekSummary: { fontFamily: body[400], fontSize: 12.5, color: color.mutedLight, marginTop: 3 },
  ...
  dayCount: { fontFamily: body[600], fontSize: 12.5, color: color.mutedLight },

  timelineRow: { flexDirection: 'row', gap: 14 },
  timeCol: { width: 52, paddingTop: 14, alignItems: 'flex-end' },
  timeStart: { fontFamily: display[600], fontSize: 14, color: color.ink, ...text.tabular },
  timeEnd: { fontFamily: body[400], fontSize: 11.5, color: color.faint, marginTop: 2, ...text.tabular },
  ...
  badge: { paddingHorizontal: 7, paddingVertical: 4, borderRadius: radius.sm - 1 },
  badgeLabel: {
    fontFamily: body[700],
    fontSize: 11,
    letterSpacing: 0.66,
  ...
  sessionMeta: { fontFamily: body[400], fontSize: 12.5, color: color.muted, marginTop: 5 },
  ...
  sessionActionLabel: { fontFamily: body[600], fontSize: 12.5, color: color.inkSoft },
});
