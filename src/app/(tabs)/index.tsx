...
import { accents, color, radius, shadow, space } from '@/theme/tokens';
...
export default function Home() {
  ...
  const shown = useMemo(() => {
    ...
    return groups.filter(
      (g) =>
        `${g.name} ${g.subject}`.toLowerCase().includes(q) || viaStudent.has(g.id),
    );
  ...
  return (
    <Screen>
      <ScrollView
        contentContainerStyle={{ paddingTop: insets.top + 10, paddingBottom: scrollInset }}
        ...
        {upNext && upNextGroup ? (
          <UpNextCard
            ...
            onAttendance={() =>
              router.push({
                pathname: '/attendance',
                params: { group: upNextGroup.id, date: upNext.date, start: upNext.start },
              })
            }
            onNotify={() =>
              router.push({ pathname: '/compose', params: { group: upNextGroup.id } })
            }
          />
        ) : null}

        <View style={styles.sectionHead}>
          <Text style={text.section}>Your groups</Text>
        ...
        {q && matchedStudents.length > 0 ? (
          <View style={styles.matchedWrap}>
            <Text style={styles.matchedLabel}>
              {matchedStudents.length} matching {matchedStudents.length === 1 ? 'student' : 'students'}
            </Text>
  ...
  return (
    <View style={styles.heroWrap}>
      <View style={styles.hero}>
        <AngledGradient colors={[color.primary, '#123AAE']} angle={150} />
        <Ring
          size={190}
          width={26}
        ...
        <View style={styles.heroActions}>
          <Press onPress={onAttendance} style={styles.heroPrimary}>
            <Icon name="check" size={15} color={color.primaryInk} />
          ...
          {slot ? (
            <>
              ...
              <Text
                style={[
                  styles.groupMeta,
                  slot.imminent && { color: color.primary, fontFamily: body[700] },
                ]}>
                {slot.label}
              </Text>
            </>
...
const styles = StyleSheet.create({
  ...
  heroDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: color.mint },
  heroEyebrow: {
    fontFamily: body[700],
    fontSize: 11,
    letterSpacing: 1.32,
  ...
  heroRoom: { fontFamily: body[600], fontSize: 12, color: 'rgba(255,255,255,0.8)' },
  heroMidRow: {
    flexDirection: 'row',
    alignItems: 'flex-end',
    justifyContent: 'space-between',
  ...
  heroPrimaryLabel: { fontFamily: body[700], fontSize: 14.5, color: color.primaryInk },
  heroSecondary: {
    flex: 1,
    height: 46,
    borderRadius: radius.field,
  ...
  heroSecondaryLabel: { fontFamily: body[600], fontSize: 14.5, color: '#fff' },
  ...
  groupMetaRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 5 },
  ...
  newGroupLabel: { fontFamily: body[600], fontSize: 14.5, color: color.muted },
